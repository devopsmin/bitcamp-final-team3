-- 현재버전 : 
-- board 이미지 첨부 테이블 변경
-- tripimg 테이블 삭제 후 이미지 varchar(255) 컬럼
-- boardimage 로 이동
...
CREATE TABLE boardimage (
	...
	boardimage_no   INTEGER      NULL,     -- 이미지번호
	boardimage_name VARCHAR(255) NOT NULL  -- 이미지
);	
...
ALTER TABLE boardimage
	ADD CONSTRAINT FK_board_TO_boardimage -- 게시판 -> 게시판이미지
	FOREIGN KEY (
	board_no -- 게시판번호
	)
	REFERENCES board ( -- 게시판
