...
-- 여행지구분
DROP TABLE IF EXISTS locationclass RESTRICT;
...
CREATE TABLE user (
  ...
  user_password      VARCHAR(32)  NOT NULL COMMENT '비밀번호', -- 비밀번호
  ...
  user_autherlize    TINYINT      NOT NULL COMMENT '권한여부', -- 권한여부
...
CREATE TABLE firstclass (
  firstclass_no   INTEGER     NOT NULL COMMENT '대분류번호', -- 대분류번호
  firstclass_code VARCHAR(50) NULL     COMMENT '분류코드', -- 분류코드
  firstclass_name VARCHAR(50) NOT NULL COMMENT '분류명' -- 분류명
)
COMMENT '여행지유형대분류';
...
ALTER TABLE firstclass
  MODIFY COLUMN firstclass_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '대분류번호';

-- 여행지정보
CREATE TABLE location (
  location_no    INTEGER      NOT NULL COMMENT '여행지정보', -- 여행지정보
  city_code      VARCHAR(50)  NULL     COMMENT '시군구코드', -- 시군구코드
  thirdclass_no  INTEGER      NULL     COMMENT '소분류번호', -- 소분류번호
  ...
  location_class INTEGER      NULL     COMMENT '여행지구분' -- 여행지구분
)
COMMENT '여행지정보';
...
CREATE TABLE secondclass (
  secondclass_no   INTEGER     NOT NULL COMMENT '중분류번호', -- 중분류번호
  firstclass_no    INTEGER     NULL     COMMENT '대분류번호', -- 대분류번호
  secondclass_code VARCHAR(50) NOT NULL COMMENT '분류코드', -- 분류코드
  secondclass_name VARCHAR(50) NOT NULL COMMENT '분류명' -- 분류명
)
COMMENT '여행지유형중분류';
...
ALTER TABLE secondclass
  MODIFY COLUMN secondclass_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '중분류번호';

-- 여행지유형소분류
CREATE TABLE thirdclass (
  thirdclass_no   INTEGER     NOT NULL COMMENT '소분류번호', -- 소분류번호
  secondclass_no  INTEGER     NULL     COMMENT '중분류번호', -- 중분류번호
  thirdclass_code VARCHAR(50) NOT NULL COMMENT '분류코드', -- 분류코드
  thirdclass_name VARCHAR(50) NOT NULL COMMENT '분류명' -- 분류명
)
COMMENT '여행지유형소분류';
...
ALTER TABLE thirdclass
  MODIFY COLUMN thirdclass_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '소분류번호';
...
CREATE TABLE locationclass (
  location_class INTEGER NOT NULL COMMENT '여행지구분' -- 여행지구분
)
COMMENT '여행지구분';

-- 여행지구분
ALTER TABLE locationclass
  ADD CONSTRAINT PK_locationclass -- 여행지구분 기본키
  PRIMARY KEY (
  location_class -- 여행지구분
  );
...
-- 여행지정보
ALTER TABLE location
  ADD CONSTRAINT FK_locationclass_TO_location -- 여행지구분 -> 여행지정보
  FOREIGN KEY (
  location_class -- 여행지구분
  )
  REFERENCES locationclass ( -- 여행지구분
  location_class -- 여행지구분
  );
